...
def count_text():
    for i in range(len(typed_text.get('1.0','end-1c').split())):
        ...
        if all_text_list[i]==typed_text_list[i]:
            n+=1
        score=customtkinter.CTkLabel(master=app,text=f"your score{n} per minute")
...
count_start_button=customtkinter.CTkButton(master=app,text="Restart",command=count_time)
count_start_button.place(relx=0.7, rely=0.2, anchor=tkinter.CENTER)

label=customtkinter.CTkLabel(master=app,text="Start typing to test your speed")
